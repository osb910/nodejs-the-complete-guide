...
const {parseDocxTm, parseTableData} = require('./util/table-converter');
// DATABASE
...
const fileFilter = (req, file, cb) => {
    cb(null, /image\/(pn|jpe?)g/.test(file.mimetype));
};
...
(async () => {
  try {
    ...
    app.listen(process.env.PORT, async () => {
      ...
      const {segments, targetLang} = await parseDocxTm(path.join(rootDir, 'data', 'doc', 'test.docx'));
